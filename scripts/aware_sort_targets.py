...
def main():
    # CLI arguments
    (
        input_filename, output_filename, obs_time, site,  airmass_plot
    ) = parse_cli_args()
    ...
    targets = [
        ...
        for event, ra, dec in zip(
            df["event"].tolist(), 
            df["ra_center"].tolist(), 
            df["dec_center"].tolist()
        )
    ]

    # Find nearest time window for observations
    log.info(
        "Estimating nearest observational window for site %s", mondy.full_name
    )
    start, end = mondy.nearest_observation_window(obs_time)
    ...
    obs_targets = mondy.observable_targets(targets, start, end)
    ...
    sorted_targets = mondy.observation_order(obs_targets, start, end)
    # log.info("All targets sorted at epoch %s (mean time)", ctr.isot)

    if airmass_plot:
        log.info("Drawing airmass plot")
        ax = site.plot_airmass(
            sorted_targets, ctr, start, end, fig_path=airmass_plot
        )
    ...
    try:
        out = {
            "observatory": {
                    "name": mondy.name,
                    "geo_lattitude": mondy.location.lat.value,
                    "geo_longitude": mondy.location.lon.value,
            },
        ...
        log.info(
            "Saved the sorted list of targets to file: %s", output_filename
        )
