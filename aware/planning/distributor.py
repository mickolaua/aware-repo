                ...
                if start is None or stop is None:
                    ...
                    try:
                        ...
                        if idx.size:
                            ...
                            planned_targets += sorted_neighbors
    ...
    def distribute(
        ...
        for tel in telescopes:
            ...
            if start is None or stop is None:
                ...
                try:
                    # First off, walk the contours and retrieve all possible 
                    # sky fields of a size of the telescope FOV
                    nside = hp.get_nside(self.hpx)
                    npix = hp.get_map_size(self.hpx)
                    nest_idx = hp.ring2nest(nside, np.arange(npix))
                    if not self.fields.get(tel.name, []):
                        if not self.day:
                            fields = mosaic_walker(
                                self.hpx, 
                                tel.fov, 
                                prob=prob, 
                                site=tel, 
                            ...
                            fields = []
                        self.fields[tel.name] = fields
                    else:
                        fields = self.fields[tel.name]

                    # Retrieve the list of fields that a telescope can observe in
                    # principal in the current night
                    obs_targets = tel.observable_targets(
                    ...
                    # Mark planned fields
                    for planned_field in obs_targets:
                        self.fields[tel.name].remove(planned_field)
