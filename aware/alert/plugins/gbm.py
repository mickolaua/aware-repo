...
__all__ = ["GBMFinPosAlertParser", "GBMAlertAlertParser", "GBMFltPosAlertParser"]
...
class GBMAlertAlertParser(AlertParser):
    ...
    def parse_alert(
        ...
        isot = Time(
            (
                root.WhereWhen.ObsDataLocation.ObservationLocation.AstroCoords.Time.TimeInstant.ISOTime.text
            ),
            format="isot",
        ).datetime
        ...
        if T90 := meta["Trig_Dur"]:
            descr += f"Trigger Duration: {T90:.3f} s\n"

        if sigma := meta["Trig_Signif"]:
            descr += f"Trigger Significance: {sigma:.1f}\n"
...
class GBMFltPosAlertParser(AlertParser):
    ...
    def parse_alert(
        ...
        info = parse_fermi_alert(
            alert_msg, GBMFltPosAlertParser.instrument, GBMFltPosAlertParser.topic
        )
        return info


class GBMGndPosAlertParser(AlertParser):
    topic = "gcn.classic.voevent.FERMI_GBM_GND_POS"
    instrument = FERMI_GBM
    ...
    def parse_alert(
        ...
        info = parse_fermi_alert(
            alert_msg, GBMGndPosAlertParser.instrument, GBMGndPosAlertParser.topic
        )
        return info


class GBMFinPosAlertParser(AlertParser):
    ...
    def parse_alert(
        ...
        info = parse_fermi_alert(
            alert_msg, GBMFinPosAlertParser.instrument, GBMFinPosAlertParser.topic
        )
        return info
